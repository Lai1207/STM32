...
extern uint16_t Num; 					//引入main.c 的變數
...
void Timer_Init(void){
	...
	TIM_ETRClockMode2Config(TIM2,TIM_ExtTRGPSC_OFF,TIM_ExtTRGPolarity_NonInverted,0x00);	//使用外部時鐘(預設)
	...
	TIM_TimerBaseStructure.TIM_Period = 10 - 1;						//ARR
	TIM_TimerBaseStructure.TIM_Prescaler = 1 - 1;					//PSC
...
{
	return TIM_GetCounter(TIM2); 									//cnt 計數值
}
